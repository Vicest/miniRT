...
/*   Updated: 2021/02/21 20:56:09 by vicmarti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long double	cylinder_collision(void *cylinder, t_vector v)
{
	...
	if (!isnan(sol2) && (sol2 < 0.0L || equals_zero(sol2)))
		sol2 = NAN;
	sol1 = fminl(sol1, sol2);
	if (isnan(sol1))
		return (NAN);
	p_m_x1 = point_at_dist(v, sol1);
	vect_sub(&p_m_x1, p_m_x1, c.pos.orig);
	if (dot_prod(p_m_x1, c.pos.dir) < 0)//|| dot_prod(p_m_x1, c.pos.dir) > c.h * c.h)
		return (NAN);
	if (dot_prod(p_m_x1, c.pos.dir) > c.h * c.h)
		return (NAN);
	return (sol1);
}
